from jigboard.JigboardTab import JigboardTab
from structure.Subsystem import Subsystem
from transmission.ComsThread import ComsThread
from Robot.Constants import Constants


class Claw(Subsystem):
    def __init__(self):
        super().__init__()
        
        self._roll_power = 0
        self._claw_roll_speed = Constants.claw_roll_speed
        self.open_claw()
        
        self.programmer_tab = JigboardTab("Programmer Board")
        
    def open_claw(self):
        self._set_clamp(Constants.claw_open) 
    
    def close_claw(self):
        self._set_clamp(Constants.claw_closed)
    
    def is_claw_open(self):
        # Check if the claw is open by checking the clamp motor position
        return self.clamp_motor == Constants.claw_open
    
    def _set_clamp(self, clamp_angle):
        self.clamp_motor = clamp_angle
        
        ComsThread().set_claw_clamp(self.clamp_motor)

    def roll_left(self):
        self._set_roll_power(-self._claw_roll_speed)

    def roll_right(self):
        self._set_roll_power(self._claw_roll_speed)

    def stop_roll(self):
        self._set_roll_power(0)
    
    def _set_roll_power(self, roll_power):
        # Set the roll motor speed based on the roll angle
        self._roll_power = roll_power
        
        # Send the command to the ComsThread to set the roll motor
        ComsThread().set_claw_roll(roll_power)
    
    def periodic(self):
        self.programmer_tab.put_string("Claw Motor Angle", 
                             "Clamp: {:.2f} Roll: {:.2f}".format(self.clamp_motor, self._roll_power))
